    ...
    <div className="min-h-screen bg-gray-50 p-8">
        ...
        {filters.map(([key, label]) => (
          <button 
            ...
            className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${
              activeFilter === key ? 'bg-orange-600 text-white' : 'bg-white text-gray-600 border hover:border-orange-300 hover:text-orange-600'
            }`}
          >
            {label}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {filteredDestinations.map(dest => (
          <Link href={`/${dest.slug}`} key={dest.slug} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all">
            <div className="relative h-48">
              <img src={dest.hero_image} alt={dest.hero_title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
            </div>
            <div className="p-6">
              <h2 className="text-xl font-bold mb-2">{dest.hero_title}</h2>
              <p className="text-gray-500 text-sm line-clamp-2">{dest.meta_description}</p>
